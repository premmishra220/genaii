...
<div style="display:flex; align-items:center; justify-content:space-between;">
  ...
  <div style="text-align:right; color:#9aaed8; font-size:12px;">
    Deploy-ready • Chat-style UI
  </div>
</div>
<hr>
""", unsafe_allow_html=True)
...
if st.button("Send"):
    if user_input and user_input.strip():
        ...
        # construct prompt: only latest user input + optional minimal context (here we choose only latest user input to avoid repetition)
        prompt_text = f"{user_input.strip()}\nAnswer in simple words only."

        # lazy-load generator once (saves repeated downloads)
        ...
        # generate response (safe prompt: only latest)
        try:
        ...
        # refresh conversation variable to include new messages
        conversation = load_users()[st.session_state.email]["chats"].get(st.session_state.current_chat, [])

# -----------------------------
# Render conversation (last 50)
# -----------------------------
conversation = load_users()[st.session_state.email]["chats"].get(st.session_state.current_chat, [])
